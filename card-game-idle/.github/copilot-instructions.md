...
- Enigma banner prompts must mirror the exact format of `Midjourney Art/Splotched Ink Replacement Prompts.md` and remain in `Midjourney Art/Enigma Manuscript Banner Prompts.md`: scope, shared style language, parameters, named sections, fenced prompt blocks with inline Midjourney parameters, and a shared negative prompt. Share the splotched ink aesthetic, but make every Enigma composition and accent palette distinct.
