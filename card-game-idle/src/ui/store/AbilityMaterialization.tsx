          ...
          {ABILITY_DEFINITIONS.map(ability => {
            ...
            return (
              <article key={ability.id} style={{ minHeight: 190, padding: 18, borderRadius: 10, border: `1px solid ${owned ? 'rgba(120,220,140,0.72)' : 'rgba(110,185,240,0.3)'}`, background: owned ? 'rgba(60,150,90,0.12)' : 'rgba(18,38,62,0.72)', boxShadow: owned ? '0 0 20px rgba(120,220,140,0.12)' : '0 10px 24px rgba(0,0,0,0.2)', display: 'flex', flexDirection: 'column' }}>
                <div style={{ color: owned ? '#9be8a8' : '#7dd4f8', fontFamily: uiTypography.display, fontSize: 17, letterSpacing: 0.6 }}>{ability.name}</div>
                ...
                <div style={{ marginTop: 14, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
                  <span style={{ color: '#f7c04a', fontSize: 11 }}>5,000 Divine Light</span>
                  <button type="button" disabled={owned || !affordable} onClick={() => purchaseAbility(ability.id)} style={{ padding: '7px 10px', borderRadius: 6, border: `1px solid ${owned ? 'rgba(120,220,140,0.5)' : 'rgba(110,185,240,0.58)'}`, background: owned ? 'rgba(120,220,140,0.12)' : 'rgba(78,160,220,0.18)', color: owned ? '#9be8a8' : '#d8f0ff', cursor: owned || !affordable ? 'not-allowed' : 'pointer', opacity: !owned && !affordable ? 0.45 : 1, fontFamily: uiTypography.body, fontSize: 10 }}>
                    {owned ? 'Owned' : affordable ? 'Materialize' : 'Insufficient'}
                  </button>
                </div>
              </article>
            );
