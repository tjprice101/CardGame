...
function CurrencyTile(props: { name: string; value: string; description: string; iconUrl?: string; glyph?: string; glyphStyle?: React.CSSProperties }) {
  return (
    <div style={{
      ...
      {props.iconUrl
        ? <img src={props.iconUrl} alt="" style={{ width: 96, height: 96, objectFit: 'contain', flexShrink: 0 }} />
        : <div style={{ width: 96, height: 96, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 32, ...props.glyphStyle }}>{props.glyph}</div>}
