...
export function calculateNeutralityBossBaseline(): BossAttackBaseline {
  ...
  const bestAin = Math.max(0, ...lightCards.map(card => card.ainAttack.baseDivineLight + (card.ainAttack.scaling.kind === 'triune' ? card.ainAttack.scaling.amount : 0)));
  const bestSoph = Math.max(0, ...lightCards.map(card => card.sophAttack.baseDivineLight + (card.sophAttack.scaling.kind === 'triune' ? card.sophAttack.scaling.amount : 0)));
  const bestBridge = Math.max(0, ...asaCards.map(card => card.bridgeAttack ? card.bridgeAttack.baseDivineLight + (card.bridgeAttack.scaling.kind === 'triune' ? card.bridgeAttack.scaling.amount : 0) : 0));
