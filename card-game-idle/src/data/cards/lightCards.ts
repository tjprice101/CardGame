...
export const lightCards: LightCardDefinition[] = lightNames.map((name, index) => {
  ...
  return {
    ...
    ainAttack: {
      ...
      description: `${ainBase} base Divine Light; scales with Collection Power.`,
      baseDivineLight: ainBase, cooldownCards: 1 + (index % 4),
      scaling: { kind: 'linear', reads: 'collectionPower', multiplier: ainScale / 1000 },
      tags: ['light', 'ain-attack'],
    },
    sophAttack: {
      id: `${id}:soph-attack`, label: 'Soph', name: 'Soph Attack',
      description: `${sophBase} base Divine Light; scales with Collection Power; consumes ${1 + (index % 5)} Limitless Light Stacks.`,
      baseDivineLight: sophBase, cooldownCards: 2 + (index % 6),
      scaling: { kind: 'linear', reads: 'collectionPower', multiplier: sophScale / 1000 },
