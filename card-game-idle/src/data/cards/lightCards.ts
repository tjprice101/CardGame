...
export const lightCards: LightCardDefinition[] = lightNames.map((name, index) => {
  ...
  return {
    ...
    name,
    description: `Light creature. Ain Attack: ${ainBase} base Oblivion, +${ainScale} scaled evenly across Limitless Light Stacks, summoned Ain Soph Aur, and Collection Power, ${1 + (index % 4)}-play cooldown. Soph Attack: ${sophBase} base Oblivion, +${sophScale} scaled evenly across the same three sources, costs ${1 + (index % 5)} Limitless Light Stack${1 + (index % 5) === 1 ? '' : 's'}, ${2 + (index % 6)}-play cooldown.`,
    artKey: artKeys[index],
    ainAttack: {
      id: `${id}:ain-attack`, label: 'Ain', name: 'Ain Attack',
      description: `${ainBase} base Oblivion; +${ainScale} scaled evenly across Limitless Light Stacks, summoned Ain Soph Aur, and Collection Power.`,
    ...
    sophAttack: {
      ...
      description: `${sophBase} base Oblivion; +${sophScale} scaled evenly across Limitless Light Stacks, summoned Ain Soph Aur, and Collection Power; consumes ${1 + (index % 5)} stacks.`,
