...
export const blazingGardenCards: CardDefinition[] = [
  ...
  buildSeraphim({
    ...
    description: 'On play: Gain 10 Radiance; Empower the next card you play. While on board: Chain grows +0.04 per card played while active',
    rarity: 'Rare',
    artKey: 'bg_ser_aureveth_noon_petal',
    bonusType: 'chain_bonus',
    bonusValue: 0.04,
    onPlayEffects: [{ type: 'radiance_gain', value: 10 }, { type: 'multiply_next' }],
  ...
  buildSeraphim({
    ...
    bonusValue: 22,
    onPlayEffects: [{ type: 'strain_gain', value: 2 }, { type: 'draw', value: 2 }, { type: 'set_chain_floor', value: 1.9 }],
  ...
  buildSeraphim({
    ...
    description: 'On play: Salvage any 1 card; Gain 14 of your dominant resource; Draw 1 card. While on board: Resource generation +14 while active',
    rarity: 'Legendary',
  ...
  buildCherubim({
    ...
    description: 'On play: Gain 12 Radiance. While on board: Adjacent active Seraphim gain +0.05 chain growth; Buffs Seraphim and Angel attacks: base +33, chain scaling +0.01, cooldown +0, multiplier x1.00',
    rarity: 'Rare',
    artKey: 'bg_cher_auric_floret_keeper',
    effects: [{ type: 'cherubim_adjacent_seraphim_bonus', bonusType: 'chain', value: 0.05 }],
    onPlayEffects: [{ type: 'radiance_gain', value: 12 }],
  }),
  buildCherubim({
    definitionId: 'bg-cher-thistleproof-chorister',
    name: 'Thistleproof Chorister',
    description: 'On play: Gain 1 Strain; Draw 1 card. While on board: Seraphim bonuses are amplified by +0.08; Buffs Seraphim attacks: base +42, chain scaling +0.05, cooldown -1, multiplier x1.00',
    rarity: 'Rare',
  ...
  buildOphanim({
    ...
    description: 'Replay last Ophanim played this turn; Gain 8 Radiance; Empower the next card you play; Draw 1 card',
    rarity: 'Epic',
    artKey: 'bg_oph_spiral_memory_bloom',
    effects: [{ type: 'copy_last_hr' }, { type: 'radiance_gain', value: 8 }],
  }),
  buildOphanim({
  ...
  buildCherubim({
    ...
    description: 'On play: Gain 2 Strain; Draw 1 card. While on board: Seraphim bonuses are amplified by +0.09; Buffs Seraphim attacks: base +66, chain scaling +0.08, cooldown -1, multiplier x1.00; Buffs Angel attacks: base +51, chain scaling +0.05, cooldown -1, multiplier x1.00',
    rarity: 'Eternal',
  ...
  buildOphanim({
    ...
    description: 'Draw 5 cards; Gain 42 Radiance; Set chain multiplier to x5.0',
    rarity: 'Infinite',
    artKey: 'bg_inf_noon_that_never_sets',
    effects: [{ type: 'draw', value: 5 }, { type: 'radiance_gain', value: 42 }, { type: 'chain_multiplier_set', value: 5.0 }],
  }),
  buildOphanim({
