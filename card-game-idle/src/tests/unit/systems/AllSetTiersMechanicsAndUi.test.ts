...
function snapshotSignature(turn: TurnState): string {
  return JSON.stringify({
    ...
    lightAnchors: turn.lightChorusAnchors ?? 0,
    trail: turn.trail ?? 0,
    scar: turn.scar ?? 0,
    thornCounter: turn.secondaryCounters?.thorn ?? 0,
    thornStack: turn.eternalStacks?.thorn ?? 0,
    mechanicalTicks: turn.mechanicalClockTicks ?? 0,
    mechanicalChimes: turn.mechanicalChimesFired ?? 0,
...
const UI_TURN_MUTATORS: Record<EngineKey, (turn: TurnState) => TurnState> = {
  ...
  mechanical: turn => ({ ...turn, strain: 6, mechanicalClockTicks: 4, mechanicalChimesFired: 1 }),
