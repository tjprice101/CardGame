...
describe('OfflineProgress', () => {
  it('returns null when offline duration is below threshold', () => {
    const result = OfflineProgress.calculate(Date.now() - 10_000, Date.now(), baseStats);
    expect(result).toBeNull();
  });

  it('calculates correct ticks for 1 hour offline', () => {
    const oneHourMs = 3600 * 1000;
    const result = OfflineProgress.calculate(Date.now() - oneHourMs, Date.now(), baseStats);
  ...
  it('caps offline progress at 8 hours', () => {
    ...
    const result = OfflineProgress.calculate(Date.now() - twelveHoursMs, Date.now(), baseStats);
