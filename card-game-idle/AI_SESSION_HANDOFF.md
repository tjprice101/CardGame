...
- Enigma banner prompts live in `Midjourney Art/Enigma Manuscript Banner Prompts.md` and must follow the exact structure of `Midjourney Art/Splotched Ink Replacement Prompts.md`: scope, shared visual language, parameter line, named asset sections, fenced prompts with inline parameters, and a shared negative prompt. Keep the splotched ink treatment consistent while making every Enigma subject, composition, symbol, and accent palette unique.
